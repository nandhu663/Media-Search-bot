...
async def filter(client, message):
    ...
    if 2 < len(message.text) < 100:    
        ...
        if len(btn) > 10: 
            ...
            if poster:
                await message.reply_photo(photo=poster, caption=f"<b>🎬 Film/Series : {search}</b>\n<b>🗂️ Total Files Found :- {(len_results)}</b>\n\n<b>© By @tvseriezzz ‌‌‌‌‎ ­  ­  ­  ­  ­  </b>", reply_markup=InlineKeyboardMarkup(buttons))
        ...
        if poster:
            ...
            await message.reply_text(f"🎬 Film/Series : {search}</b>\n<b>🗂️ Total Files Found :- {(len_results)}</b>\n\n<b>© By @tvseriezzz ‌‌‌‌‎ ­  ­  ­  ­  ­  </b>", reply_markup=InlineKeyboardMarkup(buttons))
